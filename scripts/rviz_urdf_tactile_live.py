...
class UrdfTactileLiveNode(Node):
    def __init__(self, args):
        super().__init__("rh56_urdf_tactile_live")
        self.args = args
        ...
        self.get_logger().info(f"Baseline: keep {args.side} hand/object untouched for {args.baseline_sec:.1f}s")
        samples = []
        ...
        self.get_logger().info("Baseline ready. Touch sensors and watch RViz.")
        ...
        for group, pts in sorted(by_group.items()):
            ...
            for p in pts:
                ...
                if self.args.flat_colors:
                    ...
                    marker.colors.append(rgba(0.85 * (1 - k) + base[0] * k, 0.85 * (1 - k) + base[1] * k, 0.85 * (1 - k) + base[2] * k, 1.0))
    ...
    def tick(self):
        ...
        points = tactile_points(sample, self.baseline, self.joints, self._child_joint_by_link, positions, self.args.threshold)
        self.publish_contacts(points)
        counts = {}
        for p in points:
            counts[p["sensor"]] = counts.get(p["sensor"], 0) + 1
        msg = String()
        msg.data = f"side={self.args.side} active_taxels={len(points)} top={sorted(counts.items(), key=lambda item: item[1], reverse=True)[:6]}"
...
def main():
    ...
    parser.add_argument("--marker-size", type=float, default=0.007)
