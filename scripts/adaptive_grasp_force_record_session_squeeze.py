...
def main():
    ...
    p.add_argument("--min-freeze-pos", type=int, default=900, help="не считать контакт стопом раньше этой позиции")
    p.add_argument("--squeeze-after-contact", type=int, default=350, help="на сколько дозакрыть палец после первого контакта")
    p.add_argument("--force-consecutive", type=int, default=2, help="сколько подряд force-срабатываний нужно для контакта")
    ...
    try:
        ...
        # Для сильного хвата:
        # сначала ловим контакт по FORCE_ACT,
        # потом НЕ стопаем сразу, а дозакрываем палец ещё на squeeze_after_contact.
        contact_seen_pos = [None] * 6
        force_streak = [0] * 6
        ...
        with trace_path.open("w", encoding="utf-8", newline="") as ftrace, \
            ...
            while True:
                ...
                for i in range(5):
                    ...
                    if (not args.disable_force_stop) and moved_once[i] and current[i] >= args.min_freeze_pos:
                        if fdelta[i] >= args.force_threshold:
                            force_streak[i] += 1
                        else:
                            force_streak[i] = 0

                        if contact_seen_pos[i] is None and force_streak[i] >= max(1, args.force_consecutive):
                            contact_seen_pos[i] = current[i]
                            print(
                                f"🤏 contact finger {i} {FINGER_NAMES[i]} at pos={current[i]} "
                                f"force Δ={fdelta[i]:.1f}; will squeeze +{args.squeeze_after_contact}"
                            )

                        if contact_seen_pos[i] is not None:
                            squeeze_target = min(close_targets[i], contact_seen_pos[i] + args.squeeze_after_contact)
                            if current[i] >= squeeze_target:
                                frozen[i] = True
                                freeze_reason[i] = (
                                    f"contact_pos={contact_seen_pos[i]}, squeeze_target={squeeze_target}, "
                                    f"force Δ={fdelta[i]:.1f}"
                                )
